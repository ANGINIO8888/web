...
light.getWorldPosition();
    ...
    initScene(){
        ...
        const geometry = new THREE.BoxBufferGeometry( Math.random(-.01 ,.1), Math.random(-.1 ,.01) );

        for ( let i = 0; i < 1000; i ++ ) {

            const object = new THREE.Mesh( geometry, new THREE.MeshPhongMaterial( { color: Math.random(-1,255) * 0xffffff} ) );
...
geometry.position.copy( controller.position );
        }
